...
void ProcessFile(TFile *fHad, TFile *fMu, std::function<void(const caf::SRProxy& sr)> ev_processor){
  ...
  if (!t_effTree){
    ...
    t_effTree = (TTree*)fHad->Get("caf");
    if(t_effTree) std::cout << "Warning, didn't find 'cafTree' in " << fHad->GetName() << " but did find 'caf' - using that" << std::endl;
  }
  assert(t_effTree);
  ...
  t_effValues = (TTree*)fHad->Get("effValues");
    if (!t_effValues) {
        std::cerr << "Error: t_effValues not found in file " << fHad->GetName() << std::endl;
        return;
    }
  ...
  if (!t_PosVec) {
      std::cerr << "Error: t_PosVec not found in file " << fHad->GetName() << std::endl;
      return;
  }
  ...
  if (!t_effMu) {
      std::cerr << "Error: t_effMu not found in file " << fMu->GetName() << std::endl;
      return;
  }
...
//newlyadded
  ...
  TFile* FileWithHistoInfo = new TFile("FileWithHistEtrim_CoeffsAndOAPoswithSameEff_FirstMuResultsIncludedWithVisEtrim_NuOscProc.root", "RECREATE");
  //
  //------------------------------------------------------------------------------
  //------------------------------------------------------------------------------
  //------------------------------------------------------------------------------
  ...
  { HistOAPos[i_iwritten] = new TH1D(Form("HistOAPos_FDEvt_%d", i_iwritten), Form("HistOAPos_FDEvt_%d", i_iwritten), 67, -30.5, 3);
    ...
    // start the loop with efficiencies etc
     ...
     for (Int_t i_iwritten = 0; i_iwritten<nFDEvents; i_iwritten++)
     {
       ...
       //here I would need to scale the histogram to Posc(Enu)
       //HistEtrimAllVtxXTimesCoeff[i_iwritten]->Scale(Posc(Enu))
      ...
      std::cout<<" i_iwritten = "<<i_iwritten<<" E_nu = "<<EnuTrue[i_iwritten]<<" POsc(Enu) = "<< calc->P(14,14,EnuTrue[i_iwritten])<<endl;
...
int main(int argc, char const *argv[]){
  ...
  ProcessFile(f1, f2, [&](const caf::SRProxy& sr) {
