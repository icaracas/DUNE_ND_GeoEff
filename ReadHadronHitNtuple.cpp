...
void ReadHadronHitNtuple()
{
  ...
  // Input FDroot file
  // TString FileIn = "Output_FDGeoEff_2293930_80_100throws.root";
    TString FileIn = "Output_FDGeoEff_vetoEEventsPass_TrimEEventsPass_Entry33_HadronHitDepAtvtxX7p75.root";
  //
...
//  gStyle->SetOptStat(0);
  ...
  // for (Int_t i_iwritten = 1; i_iwritten<2; i_iwritten++)
  // {
  ...
  //           auto*xy_text = new TLatex(-400,400,energy_name);
  //           xy_text->Draw();
  //
  ...
  TFile* FileWithHistoInfo = new TFile("FileWithHistoInfo_TestDelete.root", "RECREATE");
  ...
  {  cout<<" i_iwritten "<<i_iwritten<<endl;
    ...
    HistEtrimAllVtxX[i_iwritten] = new TH1D(HistEtrimAllVtxX_name, HistEtrimAllVtxX_name, 100, 0, 10000);
    ...
    for (Int_t i_ND_LAr_dtctr_pos: a_ND_off_axis_pos_vec)
    {
      for (Double_t i_ND_LAr_vtx_pos: a_ND_vtx_vx_vec)
      {
        ...
        //cout<<" i_ND_LAr_vtx_pos "<<i_ND_LAr_vtx_pos<<endl;
        ...
        for (i_entry ; i_entry < tot_size * (i_iwritten+1); i_entry++ )
        {
          ...
          // && ND_LAr_vtx_pos == 24
          if (ND_LAr_dtctr_pos == i_ND_LAr_dtctr_pos && ND_LAr_vtx_pos == i_ND_LAr_vtx_pos && ND_LAr_vtx_pos == 7.75)
          {
            cout<<" here? "<<endl;
            Int_t offset_X = i_ND_LAr_dtctr_pos;
            ...
            cout<<" it_throw_x_counter "<<it_throw_x_counter<<endl;
            ...
            for (vector<vector<float>>::iterator it_throw = CurrentThrowDepsZ->begin(); it_throw!=CurrentThrowDepsZ->end(); ++it_throw)
            {
              for (Int_t ihadronhit =0; ihadronhit < ND_Sim_n_hadronic_Edep_b; ihadronhit++)
              {
              //  cout<< "ND_LAr_dtctr_pos: " << ND_LAr_dtctr_pos << ", ND_LAr_vtx_pos: " << ND_LAr_vtx_pos << ", ithrow: " << it_throw_z_counter << ", ihadronhit: " << ihadronhit << ", hit_z: " << it_throw->at(ihadronhit) << endl;
            ...
            for (Int_t ithrow = 0; ithrow < 10; ithrow++ )
            {
              ...
              TString h_hadronhit_xy_name = Form("hadronhitXY_event_%d_OffAxis_%d_cm_LAr_%f_cm_throw_%d", i_iwritten,i_ND_LAr_dtctr_pos,i_ND_LAr_vtx_pos,ithrow);
              // TString h_hadronhit_xy_name = Form(" ");
              ...
              // HistEout[i_ND_LAr_vtx_pos]->Fill(CurrentThrowoutEnergyND);
              // HistVetoE[i_ND_LAr_vtx_pos]->Fill(CurrentThrowvetoEnergyND);
              ...
              HistEtrimAllVtxX[i_iwritten]->Fill((CurrentThrowTotE->at(ithrow) - CurrentThrowoutEnergyND), ND_GeoEff/(nthrowsToLoop*a_ND_vtx_vx_vec.size()));
              cout<<" total E: "<<CurrentThrowTotE->at(ithrow)<<" geo eff: "<<ND_GeoEff<<endl;
              HistEtrim[i_iwritten][i_vtxX_plot-1]->SetTitle(Form("TotalFD Energy = %.2f MeV", CurrentThrowTotE->at(ithrow)));
              PlotEfficiencyVsVtxX[i_iwritten]->SetTitle(Form("TotalFD Energy = %.2f MeV", CurrentThrowTotE->at(ithrow)));
              //
             ...
             c_hadronhit[n_plot]->SaveAs( TString::Format("HadronHitPlots_ND/tstc_event_%d_hadronhit_%d_throw_%d.pdf",iwritten, n_plot,ithrow ) );
        ...
        // delete HistVetoE[i_iwritten][i_vtxX_plot-1];
        // delete HistEout[i_iwritten][i_vtxX_plot-1];
      }//end vtx pos inside LAr
    }//end LAr pos
